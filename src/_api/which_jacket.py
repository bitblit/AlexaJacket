...
"""
def on_session_started(sessionStartedRequest, session) :
    logging.info("onSessionStarted requestId={0}, sessionId={1}".format(sessionStartedRequest.requestId
                                                                        , session.sessionId))
...
def lambda_handler(event, context):
    ...
    * Make sure the first answer is the correct one. Set at least 4 answers, any extras will be shuffled in.
    ...
    try :
        logging.info("event.session.application.applicationId={}".format(event.session.application.applicationId))
